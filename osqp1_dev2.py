...
def evaluate_constraints(x_k):
    cons1 = np.subtract(np.matmul(G, x_k), u)
    cons2 = np.add(np.matmul(-G, x_k), l)
    cons3 = np.subtract(np.matmul(A, x_k), b)
...
objective += penalty * (cvxpy.norm1(G * x - u) + cvxpy.norm1(-G * x + l) + cvxpy.norm1(A * x - b))
...
while penalty.value <= max_penalty:
    ...
    cons1_at_x_k, cons2_at_x_k, cons3_at_x_k= evaluate_constraints(x_k)
    cons1_grad, cons2_grad, cons3_grad = get_constraints_gradients()

    cons1_model = cons1_at_x_k + cons1_grad * p
    cons2_model = cons2_at_x_k + cons2_grad * p
    cons3_model = cons3_at_x_k + cons3_grad * p
    ...
    objective_model = objective.value + objective_grad_at_x_k.T * p + 0.5 * cvxpy.quad_form(p, objective_hess_at_x_k) + \
                      penalty * (cvxpy.norm1(cons1_model) + cvxpy.norm1(cons2_model) + cvxpy.norm1(cons3_model))
    problem_model = cvxpy.Problem(cvxpy.Minimize(objective_model), [cvxpy.norm(p, "inf") <= 2])

...
class PlannerExample:
    def __init__(self):
        ...
        self.shelf_id = self.planner.add_constraint_from_urdf("shelf", urdf_file=shelf_file, position=[-1, -1, 0.0],
                                                              orientation=p.getQuaternionFromEuler([0, 0, 1.57]))

        # table_id = self.planner.add_constraint_from_urdf("table", urdf_file="table/table.urdf",
        #                                                  position=[0.7, 1.3, 0.0])
        #
    ...
    def run(self):
        ...
        start_state["odom_y_joint"] = -0.1
        start_state["odom_z_joint"] = 0.0

        start_state["ur5_shoulder_pan_joint"] = 2.4823357809267463
        start_state["ur5_shoulder_lift_joint"] = -2.6299975516996142
        # start_state["ur5_shoulder_lift_joint"] = -2.3299975516996142
        start_state["ur5_elbow_joint"] = -1.5762726255540713
        ...
        goal_state["odom_y_joint"] = 0.0
        goal_state["odom_z_joint"] = -1.1

        goal_state["ur5_shoulder_pan_joint"] = 2.3823357809267463
        goal_state["ur5_shoulder_lift_joint"] = -2.1299975516996142
        goal_state["ur5_elbow_joint"] = -1.5762726255540713
        ...
        #
        self.planner.world.reset_joint_states(self.robot_id, goal_state.values(), start_state.keys())
    ...
    def get_collision_points(self):
        ...
        print "shelf id", self.shelf_id
        closest_points = [ClosestPointInfo(*x) for x in p.getClosestPoints(self.robot_id, self.shelf_id, 0.1)]

        for cp in closest_points:
            if cp.contact_distance < 0:
                self.planner.world.print_contact_points(cp, 0, cp.link_index_a,
                                    self.planner.world.joint_id_to_info, self.planner.world.constraint_id_to_info, "shelf")
...
def main():
    ...
    # example.load_srdf()
    example.run()
    # example.manwual_control()
    # example.connect_directly()
    # example.get_collision_points()
    while True:
        pass

                        ...
                        if closest_points[0][8] < 0:
                            ...
                            if len(ray_test_result):
                                if ray_test_result[0][0] > -1:
                                    ...
                                    link_position_in_world_frame = current_link_state[4]
                                    link_orentation_in_world_frame = current_link_state[5]
                                    link_position_in_world_frame = np.asarray(link_position_in_world_frame) * hit_ratio \
                                                                   + (1 - hit_ratio) * np.asarray(next_link_state[0])
                                    closest_point_on_link_in_world_frame = ray_hit_point
                                    ...
                                    if len(jaco1) > 0 and len(jaco2):
                                        ...
                                        current_state_jacobian = np.hstack(
                                            [jaco1.tolist(), np.asarray(jac_t).tolist(), jaco2.tolist()])
                                    ...
                                    jac_t1, jac_r1 = sim.calculateJacobian(self.robot_id, link_index,
                                                                           ...
                                                                           robot_joint_positions,
                                                                           zero_vec, zero_vec)
                                    # jaco11 = np.asarray([[[0] * len(group)] * 3] + [[[0] * len(group)] * 3] * (time_step_count - 2))
                                    ...
                                    #
                                    # if len(jaco21) > 0:
                                    #     jaco21 = np.hstack(jaco2)
                                    #     next_stat_jacobian = np.hstack([jaco11, np.asarray(jac_t), jaco2])
                                    # else:
                                    #     next_stat_jacobian = np.hstack([jaco11, np.asarray(jac_t)])
                                    ...
                                    if len(jaco11) > 0 and len(jaco21):
                                        ...
                                        next_stat_jacobian = np.hstack(
                                            [jaco11.tolist(), np.asarray(jac_t).tolist(), jaco21.tolist()])
                                    elif len(jaco11) > 0 and len(jaco21) == 0:
                                        next_stat_jacobian = np.vstack([jaco1, np.asarray(jac_t).reshape(1, 3, 7)])
                                    elif len(jaco11) == 0 and len(jaco21) > 0:
                                        next_stat_jacobian = np.vstack([np.asarray(jac_t).reshape(1, 3, 7), jaco21])
                                    ...
                                    normal_T_times_jacobian1.append(
                                        np.matmul((1 - alpha) * normal_vector.T, next_stat_jacobian))
                                    # print link_index
                                    # print res

...
class SimulationWorld(ISimulationWorldBase):
    def __init__(self, robot, use_gui=False, use_real_time_simulation=False, fixed_time_step=0.01,
    ...
    def create_constraint(self, shape, mass, position, orientation=None, size=None, radius=None, height=None):
    ...
    def add_collision_constraints(self, constraint):
    ...
    def formulate_collision_infos(self, robot_id, trajectory, group, distance=0.2):
        ...
        start_state = self.get_current_states_for_given_joints(group, robot_id)
        time_step_count = 0
    ...
    def get_current_states_for_given_joints(self, joints, robot_id):
    ...
    def execute_trajectory(self, robot_id, trajectory, step_time=None):
        ...
        for i in range(int(trajectory.no_of_samples)):
            for joint_name, corresponding_trajectory in trajectory.trajectory_by_name.items():
                sim.setJointMotorControl2(bodyIndex=robot_id, jointIndex=self.joint_name_to_id[joint_name],
